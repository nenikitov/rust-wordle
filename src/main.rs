use std::{fs::File, io::{BufReader, BufRead, self, BufWriter, Write}};
...
fn main() {
    let words = get_words("res/words_alpha.txt");
    let words: Vec<String> = words
        .into_iter()
        .filter(is_a_valid_word)
        .collect();
    write_words("res/words_filtered.txt", &words);
}

fn write_words(file_name: &str, words: &Vec<String>) {
...
fn get_words(file_name: &str) -> Vec<String> {
